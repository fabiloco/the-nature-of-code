...
export class Mover {
  ...
  constructor(p: p5) {
    ...
    this.leftThrusterPS = new ParticleSystem(p, this.location);
  }
  ...
  update() {
    ...
    if (this.p.keyIsDown(87)) {
      ...
      this.leftThrusterPS.addParticle(this.acceleration);
    }
    ...
    if (this.p.keyIsDown(83)) {
      let force = Vector.fromAngle((this.theta * -Math.PI) / 180);
      this.applyForce(force);
      this.leftThrusterPS.addParticle(this.acceleration.mult(-1));
    }
